...
def evaluate_loader(model, gumbel_scalar, loader, cce_loss):
    model.eval()
    total_correct = 0
    total_loss = 0
    total = 0
    ...
    for s in num_slots: 
        ...
        for iter in range(NUM_ITERATIONS):
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                current_acc = evaluate_loader(model, gumbel_scalar, train_loader, cce_loss)
                model.train()
                ...
                print("Epoch:", epoch, "Loss:", running_loss / len(train_loader), "Current accuracy:", current_acc)
                with open('results/myprotonet_results.txt', 'a') as f:
            ...
            model.to(DEVICE)
            print("Final accuracy... :", evaluate_loader(model, gumbel_scalar, train_loader, cce_loss))
            ...
            print("Data reward: ", sum(all_rewards) / SIMULATION_EPOCHS)
            ...
            with open('results/myprotonet_results.txt', 'a') as f:
                f.write(f"Data reward: {sum(all_rewards) / SIMULATION_EPOCHS}, Data error: {sum(all_errors) / SIMULATION_EPOCHS}\n")
